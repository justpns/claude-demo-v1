  ...
  return (
    <section className="py-24 px-6 border-t border-zinc-800/60">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <p className="text-xs uppercase tracking-widest text-violet-400 font-semibold mb-4">What participants say</p>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {quotes.map((q, i) => (
            <motion.div
              ...
              className="flex flex-col gap-5 p-6 bg-zinc-900/60 border border-zinc-800 rounded-2xl"
            >
              {/* Quote mark */}
              <svg className="w-7 h-7 text-violet-500/40" fill="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-zinc-800">
                ...
                <div>
                  <p className="text-xs font-semibold text-zinc-300">{q.name}</p>
                  <p className="text-xs text-zinc-600">{q.company}</p>
                </div>
              </div>
            </motion.div>
          ))}
